  ...
  return (
    <div className="w-full max-w-[700px] mx-auto p-10 text-center flex flex-col justify-center items-center theme-panel rounded-2xl overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="w-full flex flex-col items-center">
        ...
        {file && (
          <div className="w-full mb-6 theme-card rounded-xl p-5 shadow-sm text-left">
            <p className="text-sm font-semibold text-[var(--color-app-text)] mb-4 flex items-center gap-2">
              <Unlock className="w-4 h-4 text-[#4361ee]" />
              Enter PDF Password
            </p>
            <label className="flex flex-col gap-1.5 relative">
              <span className="text-xs font-semibold theme-muted uppercase tracking-wider">Password</span>
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-2.5 pr-10 rounded-lg border border-[var(--color-app-border)] bg-white dark:bg-gray-700 text-[var(--color-app-text)] text-sm font-medium focus:outline-none focus:border-[#4361ee] focus:ring-2 focus:ring-[#4361ee]/15 transition-all"
                />
                <button
                  type="button"
        ...
        {statusMessage && (
          <p className={`mt-6 text-[0.95rem] ${statusMessage.toLowerCase().includes("error") ? "text-red-500" : "theme-muted"}`}>
            {statusMessage}
          </p>
        )}
      </form>
